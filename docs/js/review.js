    ...
    function makeReviewScreen({ mount, userId, onGoHome, onGoWords }) {
        ...
        function applyFiltersDue(today) {
            ...
            out.sort((a, b) => {
                ...
                const pra = window.AppStorage.getWordProgress(userId, a);
                const prb = window.AppStorage.getWordProgress(userId, b);

                const isWrongA = pra?.wrong_today === true && pra?.wrong_today_date === today;
                const isWrongB = prb?.wrong_today === true && prb?.wrong_today_date === today;

                // ① 今日×は必ず下（押した順を保持）
                if (isWrongA && isWrongB) {
                    return wrongTodayOrder.indexOf(a) - wrongTodayOrder.indexOf(b);
                }
                if (isWrongA) return 1;
                if (isWrongB) return -1;

                // ② 期日超過日数（古いほど上）
                const overdueA = Math.max(0, new Date(today) - new Date(pra?.due || today));
                const overdueB = Math.max(0, new Date(today) - new Date(prb?.due || today));
        ...
        function renderCards(dueWordIds, today) {
            ...
            for (const wid of dueWordIds) {
                ...
                btnX.addEventListener("click", (e) => {
                    ...
                    // ★ 今日×を押した順を記録（重複防止）
                    const idx = wrongTodayOrder.indexOf(wid);
                    if (idx !== -1) wrongTodayOrder.splice(idx, 1);
                    wrongTodayOrder.push(wid);
